...
// Route to fetch CAT reports
app.get('/api/reports/CAT/:rollno', async (req, res) => {
  try {
    const { rollno } = req.params;
    const catReport = await CAT.findOne({ rollno });
    if (!catReport) {
      return res.status(404).json({ error: "CAT reports not found" });
    }
    res.json(catReport);
  } catch (error) {
...
// Route to fetch SEM reports
app.get('/api/reports/SEM/:rollno', async (req, res) => {
  try {
    const { rollno } = req.params;
    const semReport = await SEM.findOne({ rollno });
    if (!semReport) {
      return res.status(404).json({ error: "SEM reports not found" });
    }
    res.json(semReport);
  } catch (error) {
